...
static inline uint16_t HL(void)
{
...
static inline uint16_t BC(void)
{
    return (uint16_t) ((B << 8) | C);
}

static inline void SET_BC(uint16_t v)
{
    B = (uint8_t) (v >> 8);
    C = (uint8_t) v;
}

static inline uint16_t DE(void)
{
    return (uint16_t) ((D << 8) | E);
}

static inline void SET_DE(uint16_t v)
{
    D = (uint8_t) (v >> 8);
    E = (uint8_t) v;
}
...
static inline uint8_t mem_ixd_rd(uint16_t base, int8_t d)
{
    return mem_rd((uint16_t) (base + d));
}

static inline void mem_ixd_wr(uint16_t base, int8_t d, uint8_t v)
{
    mem_wr((uint16_t) (base + d), v);
}
...
static uint8_t cb_shift(uint8_t op, uint8_t val)
{
    ...
    case 1:
	c = val & 1;
	out = (uint8_t) ((val >> 1) | (c << 7));
	break;			/* RRC */
    case 2:{
	    uint8_t oldc = (F & FLAG_C) ? 1 : 0;
	    c = (val >> 7) & 1;
	    out = (uint8_t) ((val << 1) | oldc);
	}
	break;			/* RL */
    case 3:{
	    uint8_t oldc = (F & FLAG_C) ? 1 : 0;
	    c = val & 1;
	    out = (uint8_t) ((val >> 1) | (oldc << 7));
	}
	break;			/* RR */
    case 4:
	c = (val >> 7) & 1;
...
static int exec_one(uint8_t prefix, int have_prefix)
{
    ...
    case 0x09:{
	    uint32_t r = (uint32_t) ((use_index ? *HL_LIKE : HL()) + BC());
	    if (use_index)
		*HL_LIKE = (uint16_t) r;
	    else
		SET_HL((uint16_t) r);
	...
	return 0;
    case 0x19:{
	    uint32_t r = (uint32_t) ((use_index ? *HL_LIKE : HL()) + DE());
	    if (use_index)
		*HL_LIKE = (uint16_t) r;
	    else
		SET_HL((uint16_t) r);
	...
	return 0;
    case 0x39:{
	    uint32_t r = (uint32_t) ((use_index ? *HL_LIKE : HL()) + SP);
	    if (use_index)
		*HL_LIKE = (uint16_t) r;
	    else
		SET_HL((uint16_t) r);
...
int main(int argc, char *argv[])
{
	...
	if (rc < 0) {
	    ...
	    printf
		("A=%02X F=%02X BC=%04X DE=%04X HL=%04X IX=%04X IY=%04X PC=%04X SP=%04X\n",
		 A, F, (unsigned) BC(), (unsigned) DE(), (unsigned) HL(),
		 (unsigned) IX, (unsigned) IY, (unsigned) PC,
		 (unsigned) SP);
	    return 1;
	}
    }
